...
```json
{
  ...
  "recent_entries": [ /* last N ledger rows */ ]
}
```
...
2. **Debit** at settle (after commit tx lands) — `intent_debit` entry,
   `reserved_lamports -= quote`, `debited_lamports_total += quote`,
   charged first against `sponsored_seed_remaining`, then
   `paid_credit_remaining`.
3. **Release** if the intent is rejected post-reservation — `reservation_release`,
   `reserved_lamports -= quote`.
...
`quoted_fee_lamports = service_base_lamports + chain_cost_lamports`, scaled
by `applied_multiplier_bps / 10000` (bps → ratio). The multiplier is derived
from queue pressure and background-submitter pressure; at idle both are
`10000` (1.0×).

Defaults (devnet):

- `service_base_lamports` = 25,000 (`CTM_FEE_SERVICE_BASE_LAMPORTS`)
- `chain_cost_lamports` = 48,000 = `submit_CU_estimate × chain_cost_mul_bps / 10000`
- Hard caps: `normal_max_fee_lamports=10,000,000`,
  `emergency_max_fee_lamports=25,000,000` (rare-spike ceiling).

Under quiet conditions a 73,000-lamport fee ≈ 0.000073 SOL ≈ $0.01 at
current SOL prices.
...
| 1     | `11111111111111111111111111111111` (System)   | `SystemProgram.transfer` → deposit_address |
...
Relevant errors bots should expect from the relayer's intent-submit path
when fees are enforced:

- `insufficient fees: need=<N> available=<M>` — deposit more SOL and call
  `/fees-deposited`.
- `fee ledger disabled` — `CTM_FEE_ENFORCEMENT_MODE=disabled` on the
  relayer; no action needed on client.
...
- Re-check `/fees/status` periodically (e.g. every 1–5 min) or whenever an
  intent gets rejected with an "insufficient fees" class error.
- Treat `sponsored_seed_remaining_lamports > 0` as free head-room; only
  deposit once it's exhausted.
- `deposit.deposit_address` can change if the relayer operator rotates the
  payer — do not hardcode it client-side.
- Keep one `(user_owner, mango_account)` pair per logical bot. The ledger
  key is the pair, so using two different mango accounts with the same
  wallet means two independent balances.
...
| `CTM_FEE_ADMIN_TOKEN`                      | unset (endpoint open)                  | If set, `POST /fees-deposited` requires this in `Authorization`      |
...
| `CTM_FEE_EXECUTE_COMPUTE_UNITS_ESTIMATE`   | (see source)                           | Reveal-side CU estimate                                              |
| `CTM_FEE_RECENT_ENTRIES_CAP`               | 1,024                                  | Max rows returned in `recent_entries`                                 |

## What's Not In the Current Relayer

For clarity vs older drafts of this doc:

- There is **no** `base_fee` field on the intent submission schema. The
  relayer charges its current quote at admission; clients cannot pass a cap
  inline. Use `CTM_FEE_NORMAL_MAX_LAMPORTS` server-side to ceiling fees.
- There is **no** `/relay/fee-config` or `/relay/deposit-fees` endpoint in
  this build. The routes are `/fees/status` and `/fees-deposited` on the
  execution-engine HTTP server.
- The `AUTO` / `0.00002sol` fee-preference syntax is not parsed by the
  current relayer.
